        ...
        if($date_to_ts <= $date_from_ts){
            ...
            if ((int) $makeAndModel->id > 0 && (int) $oRentalCar->id > 0 ){
                ...
                $oBooking = CarReservation::Join('car_reservation_details', 'rental_car_reservations.id', '=', 'car_reservation_details.reservation_id')
                    ...
                    ->whereRaw("(`status` != 'confirmed' OR (`status` != 'pending'))")
...
//                    ->toSql();
    ...
    public function getCarVolumeDiscountPrice($oDiscountVolume, $search){

//        $cartData = Session::get('offers');
        $o_new_day_per_day =0 ;
        $oSetting = Setting::where('key', 'currency')->first();
        $currency = ($oSetting)?$oSetting->value:'USD';

        $data['time'] = $this->_calculateDateDiff($search->start, $search->end);
        ...
        $oPrices = $oCarModel->prices()
            ->where('car_model_prices.date_from','>=',Carbon::parse($search->start)->format('Y-m-d'))
            ->where('car_model_prices.date_to','<=',Carbon::parse($search->end)->format('Y-m-d'))
            ->get();
        ...
        $real_rental_days = $this->_getRealRentalDays(Carbon::parse($search->start), Carbon::parse($search->end));
        ...
        $price_arr = $this->_getPrices(Carbon::parse($search->start), Carbon::parse($search->end), $oCarModel);
        if($price_arr['price'] == 0)
        {
            $price_arr = $this->_getDefaultPrices(Carbon::parse($search->start), Carbon::parse($search->end), $oCarModel);
        }
...
//        if($request->input('discount_code')){
        $discount_info = $this->_getDiscountInfo($search->start, $search->end, $oCarModel->id);
//        }
